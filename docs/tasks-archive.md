...
- [x] 미반납(VS05) 상태 추가 — 입실 중인데 작업기간이 끝난 방문을 카드 회수 대상으로 표시한다. VisitOverdueService 가 매시(app.visit.overdue-cron) 일괄 전환하고 감사에 SYSTEM 으로 남긴다. 저장 시에도 같은 규칙이라 기간을 연장하면 입실 중으로 복귀. 퇴실은 VS03·VS05 둘 다에서 가능(카드를 돌려받아야 하므로). BiostarX 는 건드리지 않는다 — 작업기간이 곧 장비 유효기간이라 이미 출입이 막혀 있고, 장비 정리는 실제 반납 시점(퇴실)에 한다 — sjpark2, 2026-08-06
