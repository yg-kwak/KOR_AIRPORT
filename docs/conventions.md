...
| 상태 배지 class | `badge` + 톤 `badge-info|success|warning|error|done|none`. **색만으로 구분하지 않는다**(톤별 점 모양·테두리도 다름: ●유효 / ■완료 / ○점선=없음). 마크업은 `js/core/badge.js`(`badge.of/none/count/visitStatus`) 로 만든다 | 방문상태 `badge.visitStatus('VS03','입실 중')`, 미발급 `badge.none('미발급')` |
