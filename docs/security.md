...
- **파기 이력은 감사유형 `PURGE`(자동 파기)로 남는다** — 사람이 지운 `DELETE`(삭제)와 섞이지 않는다. 감사추적에서 **유형을 '자동 파기'로 고르면 배치 이력만** 나온다. 메뉴 칸의 `[시스템]`은 화면을 거치지 않은 기록 전체(키오스크 신청·계정 잠금 포함)라 배치만 보려면 유형으로 걸러야 한다.

**화면을 거치지 않은 기록은 메뉴 칸에 `[시스템]`으로 보인다** — 정기 파기 배치, 키오스크 방문 신청, 계정 잠금처럼 `menu_id` 가 없고 로그인 사용자도 없는(`user_id='SYSTEM'`) 기록이다. 실제 메뉴가 아니므로 `tb_menu` 에는 없고, 감사추적의 **메뉴 검색 목록에 고정 항목으로** 넣어 골라 볼 수 있게 했다(`systemOnly`).
