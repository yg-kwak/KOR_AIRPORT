...
- **키오스크(무인증 방문 신청)**: `/kiosk/**` 는 `WebConfig.EXCLUDES` 로 인증·메뉴통제 제외한 공개 경로. 방문객이 로그인 없이 인솔자·방문구역·방문객을 입력해 신청하면 임시(PT02)·신청(VS01) tb_visit 로 접수되고, 관리자가 임시인원등록에서 확인 후 카드를 발급한다(그때 BiostarX 연동). 공개 경로이므로 여기서는 카드·BiostarX 쓰기·개인정보 조회를 하지 않는다.
  - **[등록 수정]**(`POST /kiosk/visit/mine`·`POST /detail`·`PUT /kiosk/visit`, 본문 `KioskEditForm`): 인솔자 **인원ID + 성명**이 둘 다 맞는 **신청(VS01) 상태** 방문만 보고 고칠 수 있다. 세 요청 모두 `KioskVisitService.requireMine` 을 지나고, 상태·유형은 서버가 임시·신청으로 고정한다. **성명은 본문으로만** 받는다 — URL 쿼리는 공용 단말의 브라우저 이력·접근 로그에 남는다. 목록 응답은 화면이 쓰는 값만(`KioskVisitResult`) — 인솔자 성명 암호문 같은 것을 무인증 응답에 싣지 않는다. **카드가 붙었거나 BiostarX 에 올라간 사람이 하나라도 있으면 거절**한다(신청 상태여도 일부 발급이 있을 수 있고, 키오스크는 카드 회수·장비 삭제를 하지 않는다). 방문객 `personId` 는 그 방문의 명단에 있는 것만 갱신하고 나머지는 신규로 받는다(무인증 요청으로 남의 인원을 덮어쓰지 못하게). 감사 로그: 조회 `키오스크 방문 조회: {방문번호} (인솔자 {인원ID})`, 수정 `키오스크 방문 수정: …`, 불일치 `키오스크 신청 조회 불일치: 인원ID {인원ID}`(성명은 적지 않는다 — 무작위 대입이 드러나게).
