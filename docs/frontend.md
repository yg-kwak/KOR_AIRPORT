...
- **방문객 카드 태깅 = 카드 태깅 모듈**: `web/visitor/visitor-card-tag.js`. `visitCardTag.init({base, params, assign})`(화면당 1회) / `on()`·`off()`(방문객 탭 진입·이탈, 모달 닫기). 방문객 탭이 열려 있는 동안 SSE(`GET {base}/card/tag/stream`)로 **내 리더에 닿은 카드번호**를 받아 **위에서부터 차례로** 방문객에게 채운다([SCAN]은 한 장에 버튼 한 번이다). 판정은 화면에 다시 적지 않고 **후보 조회(`/cards/unassigned`)를 그대로 재사용**한다 — 후보에 없으면 그 카드는 이 방문에 못 쓴다. 카드가 연달아 오면 조회 응답이 뒤섞이므로 **한 장씩 차례로**(promise chain) 처리한다.
- **방문 인솔자 = 인솔자 모듈**: 조각 `web/visitor/visitor.html` 의 인솔자 탭(`#mgrBody`) + 선택 팝업(`#mgrModal`) + `web/visitor/visitor-manager.js`. `visitManagers.init('{후보목록URL}')`(화면당 1회) / `reset()` / `set(rows)` / `get()`(저장 payload) / `close()`. **목록을 이 모듈이 쥔다** — `visitor.js` 는 저장할 값만 `get()` 으로 가져간다(파일 크기 제한으로 분리). 후보·목록에 **소속을 표기**하지만 방문에 저장하지 않는다(정규인원이 원천 — `Affiliations`). 성명 완전일치로 찾아 동명이인이 나란히 나오므로 소속이 유일한 단서다.
