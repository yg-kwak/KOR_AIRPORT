...
- **소리 안내(실시간 이벤트·이벤트 로그)**: 인증마다 결과를 소리로 알린다. 엔진은 `core/auth-sound.js` 한 벌이다 — `authSound.attach(버튼)`(화면당 1회) / `authSound.play(granted)`. 켜기/끄기 설정(`localStorage.monitorSound`)도 그 안에 있어 두 화면이 같은 값을 쓴다. **미리 만들어 둔 음성 파일**(`static/sound/auth-granted.wav`·`auth-denied.wav`)을 화면 로드 때 받아 디코딩해 두고 WebAudio 로 재생한다.
...
- **이벤트 로그(902) = 실시간 이벤트(901)와 같은 이벤트, 다른 배치**: `web/monitor/eventLog.html` + `js/web/monitor/eventLog.js`. 다른 점은 둘이다 — 단말기를 **여러 대** 한 화면에서 보고(`?deviceId=A&deviceId=B`), 지난 인증을 옆으로 늘어놓는 대신 **세로로 쌓아 스크롤**한다(최대 15건, 최근이 위). 고른 단말기는 `localStorage.monitorLogDevices` 에 남겨 다른 메뉴에 갔다 와도 그대로 이어 받는다 — 늘 켜 두는 화면이라 매번 다시 고르는 것이 가장 번거롭다. 서버(`MonitorService`)·소리(`authSound`)는 901 과 한 벌이고, 결과 배너에 **어느 단말기**인지 함께 적는다(여러 대를 보므로 결과만으로는 어느 문인지 알 수 없다).
- **BiostarX 장치 참조 = 장치 팝업**: 조각 `fragments/components/device-picker-modal` 포함 후 `const sel = await devicePicker.open('{목록URL}')` → `{id, name}`. **목록 URL 을 인자로 준다** — 화면마다 자기 `menu_id` 로 권한을 확인해야 해서 엔드포인트가 다르다(사용자관리 `/system/loginUser/biostarDevices`, 실시간 이벤트 `/monitor/event/devices`). 응답은 배열·`{success,message,devices}` 두 모양을 모두 받는다.
  **여러 대 모드**: `devicePicker.open(url, { multiple: true, selected: ['1','2'] })` → `[{id,name}, ...]`. 이벤트 로그(902)가 쓴다. `selected` 를 주면 미리 체크된 채로 열린다 — 지금 보고 있는 것이 안 보이면 고르던 것을 통째로 다시 골라야 한다. 검색으로 가려진 선택도 유지된다.
...
- **방문객 카드 태깅 = 카드 태깅 모듈**: `web/visitor/visitor-card-tag.js`. `visitCardTag.init({base, params, assign})`(화면당 1회) / `on()`·`off()`(방문구분에 인원이 있을 때 켜고, 차량만이거나 모달을 닫으면 끈다). 방문객 칸이 보이는 동안 SSE(`GET {base}/card/tag/stream`)로 **내 리더에 닿은 카드번호**를 받아 **위에서부터 차례로** 방문객에게 채운다([SCAN]은 한 장에 버튼 한 번이다). 판정은 화면에 다시 적지 않고 **후보 조회(`/cards/unassigned`)를 그대로 재사용**한다 — 후보에 없으면 그 카드는 이 방문에 못 쓴다. 카드가 연달아 오면 조회 응답이 뒤섞이므로 **한 장씩 차례로**(promise chain) 처리한다.
- **방문 인솔자 = 인솔자 모듈**: 조각 `web/visitor/visitor.html` 의 인솔자 칸(`#mgrBody`) + 선택 팝업(`#mgrModal`) + `web/visitor/visitor-manager.js`. `visitManagers.init('{후보목록URL}')`(화면당 1회) / `reset()` / `set(rows)` / `get()`(저장 payload) / `close()`. **목록을 이 모듈이 쥔다** — `visitor.js` 는 저장할 값만 `get()` 으로 가져간다(파일 크기 제한으로 분리). 후보·목록에 **소속을 표기**하지만 방문에 저장하지 않는다(정규인원이 원천 — `Affiliations`). 성명 완전일치로 찾아 동명이인이 나란히 나오므로 소속이 유일한 단서다.
- **방문 카드 선택 = 방문 카드 팝업**: 조각 `web/visitor/visitor.html` 의 `#vcpModal` + `core/components/card-picker-visit.js`. `visitCardPicker.open({kind:'vis'|'car', listUrl, scanUrl, exclude, params, onPick})`. 방문객(`vis`)은 검색+SCAN, 차량(`car`)은 검색만. **`params` 는 함수**로 준다 — 조회·스캔 요청에 그때의 화면 값(방문유형·선택한 출입그룹)을 함께 보내, 서버가 **이 방문에 맞는 카드만** 돌려준다(배열이면 `acGroupIds=1&acGroupIds=2` 로 편다). 카드명칭 규칙은 아래 [카드 이름 규칙](conventions.md) 참고.
...
- **엑셀 일괄등록 = 공용 엑셀 등록 컴포넌트**: 조각 `fragments/components/excel-import-modal :: excelImportModal` + `core/components/excel-import.js`. 툴바 `엑셀 등록` 버튼에서 `excelImport.open({baseUrl, hint, onDone})`. 모달 안에서 양식 다운로드(`GET {baseUrl}/excel/template`) → 파일 선택(파일명 미리보기, 선택 즉시 전송 안 함) → 업로드(`POST {baseUrl}/excel/import`, FormData `file`). 서버는 행마다 기존 create 를 **행 단위 독립 트랜잭션**(self 프록시)으로 호출해 `{success,fail,errors[]}` 반환, 성공분 있으면 onDone(목록 갱신)+모달 닫기. 기관(`/company/company`)·정규인원(`/person/person`)이 공유한다. 업로드는 `FormData`(`file`)로 `POST .../excel/import`, 서버는 행마다 기존 `create` 를 **행 단위 독립 트랜잭션**으로 호출(한 행 실패가 나머지를 막지 않음)하고 `{success, fail, errors[]}` 를 돌려준다. 화면은 성공/실패 건수 + 행별 사유(앞 5건)를 토스트로 요약하고 성공분이 있으면 목록 갱신. 필수값·중복·암호화·외부연동은 create 규칙을 그대로 재사용한다. `open` 에 `option: {name, label, hint}` 를 주면 파일 칸 아래 체크박스 하나가 붙고 업로드 때 `{name}=true|false` 로 함께 간다(열 때마다 꺼진 채 시작). 정규인원의 **[기존 인원 갱신]**(`updateExisting`)이 그것이다 — 켜면 **갱신만** 한다 — 있는 인원ID 행은 `PersonImportUpdateService` 가 **엑셀에 값이 있는 열만** 갱신하고 빈 칸은 그대로 두며, 없는 ID 는 신규로 만들지 않고 실패로 남긴다(사진·출입그룹·카드·근거문서는 엑셀에 없으니 손대지 않는다, 전용 `updateBasics`). 결과의 `updated` 가 있으면 `등록 N건 / 갱신 M건` 으로 보인다.
...
- **일시 칸(24시간)**: `<input type="text" data-datetime>` + `core/date-time.js`(정규인원 출입기간·임시/장기 작업기간·키오스크). `.value` 는 서버 형식 그대로라 화면 스크립트는 바뀌지 않는다(`conventions.md`).
...
**출입증번호 칸에는 카드명칭**을 적는다(번호가 아니다). 카드가 회수된 뒤면 마지막 카드번호로 되짚어 명칭을 찾고, 그 카드마저 없으면 번호를 남긴다.

**출입자소속 칸은 소속만** 적는다 — 주소 입력란이 없어 `/` 를 붙이지 않는다.
