...
- **엑셀 일괄등록 = 엑셀 등록 모달**: 목록 툴바의 `엑셀 등록` 버튼이 모달을 연다. 모달 안에서 `양식 다운로드`(빈 헤더 xlsx, `GET .../excel/template`) → 파일 선택(숨김 `input[type=file].file-input`) → `업로드` 순서. 선택 즉시 업로드하지 않고 업로드 버튼을 눌러야 전송(파일명 미리보기). 업로드는 `FormData`(`file`)로 `POST .../excel/import`, 서버는 행마다 기존 `create` 를 **행 단위 독립 트랜잭션**으로 호출(한 행 실패가 나머지를 막지 않음)하고 `{success, fail, errors[]}` 를 돌려준다. 화면은 성공/실패 건수 + 행별 사유(앞 5건)를 토스트로 요약하고 성공분이 있으면 목록 갱신. 필수값·중복·암호화·외부연동은 create 규칙을 그대로 재사용한다.
