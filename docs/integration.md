...
설정관리 → **BiostarX 가져오기**. `PersonImportBiostarService` · `BiostarImportAdapter`(읽기 전용).
...
- **이미 등록된 인원**: 건너뛴다. 덮어쓰면 화면에서 채운 생년월일·신원조회 같은 값이 날아간다.
- **출입그룹**: `tb_ac_group.biostar_ac_id` 에 매핑된 것만 가져온다.

카드·얼굴·출입권한은 화면에서 **항목별로 골라** 가져온다. 가져온 인원은 `biostar_user_id` 를 채워
'연동 완료'로 표시하므로, 이후 수정은 `update` 경로를 탄다(중복 생성 없음).

장비에 없는 값(생년월일·신원조회·보안교육)은 비워 두고 상태는 `신규(01)` 로 넣는다.
직위는 장비 `user_title` 과 공통코드(UT) **이름이 같을 때만** 맞춘다.

**먼저 매핑을 끝내라** — 기관과 출입그룹이 맞지 않으면 아무도 안 들어오거나 권한이 비어 들어온다.
[미리보기]로 대상과 건너뛴 사유를 확인한 뒤 실행한다.
...
- **카드만으로 인증하면 인증 사진이 없다** — 장비가 얼굴을 찍지 않는다. 화면은 빈칸 대신 **카드 그림**을 세워 "사진이 없는 것"과 "인증이 안 된 것"을 구분한다(등록 사진이 없을 때도 같다).
