...
- **기관차량 카드**(`/company/companyCar`): 차량용 카드도 `POST /api/cards` 로 등록한다(카드종류 CSN, 우리 카드구분은 **차량 고정**·패스구분 미사용). 차량은 BiostarX 사용자가 아니므로 **사용자 payload 의 cards[] 부여는 하지 않는다** — 카드 자체만 등록된다. 회수는 `tb_card.car_id=NULL`(삭제 아님).
