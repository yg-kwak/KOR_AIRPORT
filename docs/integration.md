...
- **정규인원 ↔ BiostarX 사용자**(`/person/person`): 인원(`tb_person`, `person_type='PT01'`) 등록 시 BiostarX 사용자를 생성한다. 어댑터: `BiostarUserAdapter`.
  ...
  - **실패 정책**: **등록(create)은 BiostarX 사용자 생성이 성공해야 저장**한다 — 실패(설정 없음 / 소속 기관에 `biostar_group_id` 없음 / 장비 오류)면 트랜잭션을 롤백하고 사유를 예외로 알린다(장비엔 없고 DB엔 있는 유령 인원 방지). 소속 기관에 그룹이 없으면 BiostarX 호출 전에 막고 "기관을 먼저 동기화하라"고 안내한다. **수정(update)은 기존대로 연동 실패해도 저장을 유지**하고 경고. 성공 시 `tb_person.biostar_user_id`=인원ID.
