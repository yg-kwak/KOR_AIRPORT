...
- **정규인원 ↔ BiostarX 사용자**(`/person/person`): 인원(`tb_person`, `person_type='PT01'`) 등록 시 BiostarX 사용자를 생성한다. 어댑터: `BiostarUserAdapter`.
  ...
  - **방문객 카드 태깅(소켓)**: 미등록 카드를 리더에 대면 장비가 인증 실패 `4354 VERIFY_FAIL_CARD` 를 올리는데, 그 이벤트의 **`user_id` 자리에 읽은 카드번호**가 담긴다(= `tb_card.biostar_card_value`). 이미 사람에게 발급된 카드는 인증 **성공**(4102/4106)이 뜨고 그 자리에 인원ID 가 오므로 **4354 만** 본다. 내 리더에서 읽은 것만 쓴다(`tb_login_user.dev_id`) — 리더가 여럿이라 거르지 않으면 남이 지나가며 찍은 카드가 배정된다. (`CardTagService`)
  - **소켓은 하나, 보는 쪽은 채널로 나눈다**: `BiostarEventSocket.start(..., channel, sink, statusListener)` / `stop(channel)`. 지금 채널은 둘 — `monitor`(실시간 이벤트 화면)와 `cardTag`(방문객 카드 태깅). 채널마다 소켓을 열면 BiostarX 세션이 갈라져 한쪽이 다른 쪽을 끊고, 반대로 채널 하나가 떠났다고 소켓을 닫으면 남은 쪽이 조용히 멈춘다. **마지막 채널이 떠날 때만** 닫는다.
  - **사용자 생성**: `POST /api/users` — `user_group_id`=`tb_company.biostar_group_id`, `disabled`=`tb_common`(PS).code_tag, `user_title`=`tb_common`(UT).code_name, `access_groups`=선택한 `tb_ac_group.biostar_ac_id` 목록, **`department`=허가구역 번호**(예: 인원구역 1·2·4 → `"124"` — `AirPort.common.AccessAreas`), `credentials.visualFaces`=얼굴 3종. 사진/얼굴은 `tb_person_photo` 에도 저장.
  ...
  - **비활성 상태면 장비의 얼굴을 지운다**: 상태(`tb_common` PS)의 `code_tag='true'`(정지·퇴사·회수·분실)로 저장하면 `disabled=true` 와 함께 **사진·인증얼굴·템플릿을 비워** 보낸다(`credentials.visualFaces=[]`). 출입만 막고 생체정보를 장비에 남겨 두면 상태를 되돌리는 순간 예전 얼굴로 문이 열린다.
    **우리 DB(`tb_person_photo`)는 지우지 않는다** — 그 사진은 출입증 발급(카드 인쇄)용이다. 함께 지우면 정지가 풀렸을 때 카드를 다시 찍을 수 없어 사람이 사진을 새로 받아 와야 한다. 사진을 없애려면 화면에서 얼굴을 비우고 저장한다.
    되돌릴 수 없으므로 **화면이 저장 전에 확인을 받는다**(어떤 상태가 비활성인지는 서버가 `PAGE_DISABLED_STATUS` 로 내려준다 — 화면에 코드를 박지 않는다). 상태를 되돌리면 **장비 얼굴은 다시 등록해야 한다**(출입증 사진은 그대로 있다).
...
- **출입자 정보의 값은 인원 구분에 따라 출처가 갈린다** — 정규(PT01)는 사람에게 붙은 값, 그 밖은 **방문 단위** 값이다.

  | 항목 | 정규(PT01) | 임시·장기·상주·순찰·대여 |
  |---|---|---|
  | 허가 구역 | `tb_person_ac_group` | `tb_visit_ac_group` (가장 최근 방문) |
  | 허가 기간 | `tb_person.access_start_dt`~`access_end_dt` | `tb_visit.work_start_dt`~`work_end_dt` (가장 최근 방문) |

  방문객의 두 값은 **같은 방문**에서 나와야 한다. 기준이 갈리면 화면에서 구역과 기간이 **서로 다른 방문**의 값으로 섞여 보인다.
  그래서 방문 선택 기준은 mapper 의 `<sql id="latestVisitOfPerson">` **조각 하나가 원천**이고 두 쿼리가 함께 `<include>` 한다. `MonitorPeriodRuleTest` 가 두 쿼리 모두 그 조각을 쓰는지 지킨다 — 이 규칙은 SQL 안에만 있어 목(mock) 기반 단위 테스트로는 깨져도 초록이다.

  허가 기간은 정규·방문 **같은 형식**으로 `2026-01-01 09:00:00 ~ 2037-12-31 23:59:00` 이다. 두 인원의 값을 나란히 볼 때 형식이 갈리면 같은 뜻인지 매번 되짚어야 한다.
  - **초까지 쓴다.** 출입 판정이 초 단위라, 경계에 걸린 사람을 두고 "왜 막혔나"를 볼 때 분까지만으로는 답이 안 나온다. 값은 DB 에서 초까지 읽는다(`CONVERT(varchar(19), …, 126)`) — 없는 초를 `:00` 으로 지어내지 않는다.
    ⚠️ 정규인원 조회는 목록·수정 화면이 쓰는 `listColumns`(분까지, `varchar(16)`)와 **별도 쿼리**(`selectForMonitor`)다. 등록·수정 화면의 `datetime-local` 입력이 분까지만 받으므로 그쪽을 초까지로 바꾸면 화면이 값을 못 채운다. 이 화면 전용 조회는 성명·소속·기관명·출입기간을 **한 번에** 읽는다 — 인증 1건마다 도는 자리라 왕복이 그대로 곱해진다.
  - 값이 길어 좁은 화면에서는 **두 줄로 접힌다**. 접히는 자리는 가운데 ` ~ ` 하나뿐이다 — 시작·종료를 각각 `<span>`(`white-space: nowrap`)으로 묶는다(`event.js periodHtml`). 묶지 않으면 **붙임표(`-`)가 줄바꿈 자리로 쓰여** `2026-08-04` 한 덩어리가 두 줄에 나뉜다.
  - 한쪽만 비어 있어도 있는 쪽은 보여 준다(`2026-08-04 10:31:00 ~ ?`). 기간이 반쯤 비었다는 사실 자체가 현장에서 확인할 거리다.
...
- **사진이 안 보일 때**: `MonitorEnrichService` 가 인증 한 건마다 남기는 로그로 어디서 끊겼는지 바로 안다.
