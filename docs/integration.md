...
- 인쇄 배치는 **인쇄영역을 가로·세로 모두 꽉 채운다**(stretch-fill, 비율 유지 아님) — 카드 규격 디자인이라 왜곡은 무시할 수준이고, 오른쪽/아래 흰 여백을 없앤다.
...
- **인쇄 배율 보정**: 기본 1.0 이면 인쇄영역에 딱 채운다. 프린터 하드웨어 여백으로 가장자리가 살짝 남으면 `card-print.scale`(기본 1.0)을 1보다 키워 경계를 넘겨 확대(풀블리드, 가장자리 잘림). 채움 크기에 곱한다.
