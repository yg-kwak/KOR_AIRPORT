...
### 로그 — 두 종류를 함께 본다
운영 설정 파일에는 **`LOG_PATH` 한 줄**이면 된다. 형식·분할·보관은 jar 기본값이고, 접근 로그 세부는 `config/AccessLogConfig` 에 고정돼 있다(끄기: `ACCESS_LOG=false`, 보관기간: `LOG_KEEP_DAYS`).

| 파일 | 무엇이 남나 | 언제 자라나 |
|------|-------------|-------------|
| `cjairport.log` | 기동 · 오류 · 경고 + **업무 이력**(메뉴 접속·조회·등록·수정·삭제·로그인) | 사용자가 쓰면 계속 |
| `access.{날짜}.log` | 요청 1건 = 1줄 (IP·URL·상태·처리시간) | 요청이 올 때마다 |

업무 이력은 `AuditService` 가 DB(`tb_system_log`)와 파일에 **함께** 남긴다 — 감사추적 화면을 열지 않아도 서버에서 무슨 일이 일어나는지 파일로 확인할 수 있다(`AUDIT` 로거, 사용자ID 는 `%X{userId}` 로 함께 찍힘).

> 과거에는 정상 조회·저장이 아무 로그도 남기지 않아, 종일 잘 돌아도 파일이 자라지 않고 **자정이 지나도 새 파일이 생기지 않았다**(롤오버는 '날짜가 바뀐 뒤 첫 로그가 기록될 때' 일어난다). 시스템이 멈춘 것으로 오인하기 쉬웠다.
