...
- **상태 배지(`.badge`)**: 목록의 상태·유무 표시. 톤=info(파랑, 수량·정보) / success(녹, 유효·사용) / warning(황, 처리 대기) / error(적, 오류) / done(회색 사각점, 완료) / none(회색 점선+빈 점, 없음·미발급). **색약 대비로 색 외에 점 모양·테두리도 함께 다르게** 한다. 코드→톤 매핑은 `js/core/badge.js` — 방문상태 VS01 신청=warning/VS03 입실 중=success/VS05 미반납=error(회수 필요)/VS04 퇴실 완료=done, 카드상태 CS01 정상=success/CS02 분실=error/CS03 반납=done/CS04 정지=warning/CS05 폐기=none, 인원상태 01 신규·05 재발급=success/02 정지=warning/06 분실=error/03 퇴사=done/04 회수=none.
