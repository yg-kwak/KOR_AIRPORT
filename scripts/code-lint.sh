...
echo "== [4] 컨트롤러 @RequestMapping(/{영역}/{stem}) ↔ tb_menu.menu_url 일치 =="
...
done < <(grep -rhoE '@RequestMapping\("/[a-zA-Z]+/[a-zA-Z]+"\)' src/main/java/AirPort/controller | grep -oE '/[a-zA-Z]+/[a-zA-Z]+')
