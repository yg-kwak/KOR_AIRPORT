...
check "인원 엑셀 양식 다운로드" 200 "$(curl -s -b "$CK_A" -o /dev/null -w '%{http_code}' "$BASE_URL/person/person/excel/template")"
check "인원 엑셀 업로드 빈파일 거절" 0 "$(A -F 'file=@/dev/null' "$BASE_URL/person/person/excel/import" | grep -q '"success":false' && echo 0 || echo 1)"
...
check "미발급 필터(인원·차량 공통)" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/card/card/list?assigned=N&size=5")"
...
check "기관차량 사용유무 필터" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/list?useYn=Y&size=5")"
check "기관 정규인원(차량관리자)" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/managers?companyCode=SMKCO1")"
check "미할당 차량 목록" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/unassigned?keyword=")"
check "차량 출입구역 조회" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/acCodes?carId=999999")"
check "기관의 차량 목록" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/cars?companyCode=SMKCO1")"
check "기관 필수 400" 400 "$(A -H 'Content-Type: application/json' -X POST --data '{"carNo":"99A9999","carName":"x","carType":"CT01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar")"
check "차량번호 필수 400" 400 "$(A -H 'Content-Type: application/json' -X POST --data '{"companyCode":"SMKCO1","carName":"x","carType":"CT01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar")"
check "깨진 JSON 본문 400" 400 "$(A -H 'Content-Type: application/json' -X POST --data '{"carNo":' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar")"
check "없는 차량 삭제 404" 404 "$(A -X DELETE -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar?carId=999999")"
check "카드발급 카드번호 필수 400" 400 "$(A -H 'Content-Type: application/json' -X POST --data '{"carId":999999,"cardName":"x","cardStatus":"CS01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/card")"
check "없는 차량 카드발급 404" 404 "$(A -H 'Content-Type: application/json' -X POST --data '{"carId":999999,"cardNo":"1","cardName":"x","cardStatus":"CS01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/card")"
check "없는 카드 회수 404" 404 "$(A -X DELETE -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/card?cardId=999999")"
...
if [ "$VCODE" = "302" ]; then
  ...
  check "viewer 기관차량 조회 허용" 200 "$(V -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/list?size=1")"
  check "viewer 기관차량 등록 403" 403 "$(V -H 'Content-Type: application/json' -X POST --data '{"companyCode":"SMKCO1","carNo":"1","carName":"x","carType":"CT01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar")"
  check "viewer 차량카드 발급 403" 403 "$(V -H 'Content-Type: application/json' -X POST --data '{"carId":1,"cardNo":"1","cardName":"x","cardStatus":"CS01"}' -o /dev/null -w '%{http_code}' "$BASE_URL/company/companyCar/card")"
