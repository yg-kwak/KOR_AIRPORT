...
check "기관 정규인원(차량관리자)" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/car/managers?companyCode=SMKCO1")"
check "미할당 차량 목록" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/car/unassigned?keyword=")"
check "차량 출입구역 조회" 200 "$(A -o /dev/null -w '%{http_code}' "$BASE_URL/company/car/acCodes?carId=999999")"
