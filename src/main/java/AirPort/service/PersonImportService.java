...
/**
 * 정규인원 엑셀 일괄등록 — 행마다 {@link PersonService#create}를 호출한다(행 단위 독립 트랜잭션이라 한 행 실패가 나머지를 막지
 * 않는다). <b>사용자권한·카드정보·얼굴은 제외</b>(엑셀로 다루지 않는다). 나머지 검증·ARIA 암호화·BiostarX 동기화는 create 규칙을
 * 그대로 재사용한다. (docs/backend.md)
 */
@Service
