...
(function () {
  ...
  function bind() {
    ...
    if ($('btnExcelImport')) $('btnExcelImport').addEventListener('click', () => excelImport.open({
      baseUrl: BASE,
      hint: ['양식을 내려받아 카드정보를 채운 뒤 업로드하세요.',
        '<b>카드번호·카드구분·카드명칭</b>은 필수입니다.',
        '카드구분·패스구분·카드상태는 공통코드 ID(예: CDT01, PT01, CS01)로 입력합니다.',
        '카드상태를 비우면 정상(CS01)으로 등록됩니다.',
        '인원 카드는 BiostarX 장비 등록까지 성공해야 저장되며, 미발급 상태로 들어갑니다.',
        '<b>2행은 예시</b>입니다 — 지우거나 덮어써서 입력하세요(그대로 두면 등록 시 건너뜁니다).'],
      onDone: load,
    }));
