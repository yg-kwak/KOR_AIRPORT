...
## 📋 백로그 (할일)
- [ ] 공통코드관리(tb_common) 코드 등록, 수정 규칙 변경 - 사용자가 입력하는 코드구분ID는 시스템 어디에서 사용할지를 정해진 코드이다 그렇기 때문에 사용자가 신규로 코드구분ID를 만들어도
      사용할 일이 없다 그렇기 때문에 tb_common에 user_input이 Y인 코드구분ID값만 선택할 수 있어야하고 코드구분명은 자동으로 따라와서 사용자가 입력, 수정할 수 없어야된다.
